...
public class BANDPixels {
	...
	public void initHistograms(String hipoFile, double adcRange) {
		...
		// two histograms for each layer for the global offset for paddle 1 in sector 2
		iid = Integer.toString(id)+"_L_"+"_all_C_"+"_0_LR";
		H2_a_Hist.add(0, 0, 0, new H2F("fadc_gm_tof_"+iid,		300,0.,30000,	1000,-20.,20.));
		H2_t_Hist.add(0, 0, 0, new H2F("tdc_gm_tof_"+iid,		300,0.,30000,	1000,-20.,20.));
		...
		for (int is=1; is<(bc.IS2-bc.IS1+1) ; is++) {
			...
			for( int ip = 1 ; ip<nstr[is-1]+1; ip++) {
				...
				if( (is == 3 || is == 4) && id!=4 ) {
					H2_a_Hist.add(is, ip, 0, new H2F("fadc_gm_tof_"+iid,		300,0.,30000,	1000,-20.,20.));
					//  ToF - Ref vs GM: x-axis is GM of sqrt(L*R) ADC and y-axis is (L+R)/2. - RF time using 
					//	times from TDC
					H2_t_Hist.add(is, ip, 0, new H2F("tdc_gm_tof_"+iid,		300,0.,30000,	1000,-20.,20.));
				}
				else {
					H2_a_Hist.add(is, ip, 0, new H2F("fadc_gm_tof_"+iid,		300,0.,30000,	1000,-20.,20.));
					//  ToF - Ref vs GM: x-axis is GM of sqrt(L*R) ADC and y-axis is (L+R)/2. - RF time using 
