...
public class BANDDet extends FCDetector {
    ...
    public void initDetector(int is1, int is2) {
		...
		//System.out.println("bandPix.length = "+bandPix.length);
        
        for(int id=0; id<bandPix.length; id++){
	        for(int is=is1; is<is2; is++) {
	            //System.out.println("id = "+id+" / is-1 = "+(is-1)+" / bandPix[id].nstr[is-1] = "+bandPix[id].nstr[is-1]);
...
//        int off = (layer-1)*ftofPix[det].nstr;
        ...
        for(int j = 0; j < 4; j++){
        	//System.out.println("creating shape - type: "+DetectorType.BAND+" sector: "+sector+" layer: "+layer+" paddle: "+paddle+" order: "+order+" -bandPix[det].band_xpix[j][paddle+off][sector-1]: "+-bandPix[det].band_xpix[j][paddle+off][sector-1]+"-bandPix[det].band_ypix[j][paddle+off][sector-1]"+-bandPix[det].band_ypix[j][paddle+off][sector-1]);
